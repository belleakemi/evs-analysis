...
for (transtorno in transtorno_tabela) {
  ...
  if (!dir.exists(paste0("analysis_results/", tag_perc, "", tag_transtorno))) { dir.create(paste0("analysis_results/", tag_perc,  "", tag_transtorno)) }
  
  # LOG
  log_file_name <- paste0("analysis_results/", tag_perc,"", tag_transtorno, "/log_analise", tag_perc, "",  tag_transtorno, "", Sys.Date(), ".txt")
  ...
  tryCatch({
    ...
    tryCatch({
     ...
     # write.csv(tabela_desenho, paste0("analysis_results/", tag_transtorno, "", tag_perc, "/tabela_desenho", tag_transtorno, "_", tag_perc), row.names = F, quote = F)
    ...
    png(paste0("analysis_results/", tag_perc, "", tag_transtorno, "/plot_diagnostics_size_GAUSSIAN", tag_perc, "_",  tag_transtorno, ".png"), width = 3000, height = 1200, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 0.8); plotQQunif(residuos_gaussiano, main = "A) QQ Plot - Size (Gaussian)", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_gaussiano, main = ""); title("B) Residuals vs. Predicted – Size (Gaussian)", line = 2); dev.off()
    ...
    for(v in vars){
      ...
      filename <-paste0("analysis_results/", tag_perc, "", tag_transtorno, "/plot_diagnostics_size_GAUSSIAN", v, "", tag_perc, "",  tag_transtorno, ".png")
    ...
    cat("\nDiagnostics for Gaussian Model:\n"); print(tabela_diag_gaussiano); write.csv(tabela_diag_gaussiano, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_diagnostics_dharma_size_GAUSSIAN", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
    ...
    anova_size <- Anova(modelo_size, type = "III"); df_anova_size <- as.data.frame(anova_size); df_anova_size$Significance <- ifelse(df_anova_size$Pr(>Chisq) < alfa, "Significant", "Not Significant"); print(df_anova_size); write.csv(df_anova_size, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_anova_size", tag_perc, "_", tag_transtorno, ".csv"))
    ...
    if (p_interacao_size < 0.05) {
      ...
      emm_size_p1 <- emmeans(modelo_size, specs = pairwise ~ coluna_traj | wave, adjust = "tukey"); df_emm_size_p1 <- as.data.frame(emm_size_p1$contrasts); df_emm_size_p1$Significance <- ifelse(df_emm_size_p1$p.value < alfa, "Significant", "Not Significant"); print(df_emm_size_p1); write.csv(df_emm_size_p1, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_wave_size", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
      emm_size_p2 <- emmeans(modelo_size, specs = pairwise ~ wave | coluna_traj, adjust = "tukey"); df_emm_size_p2 <- as.data.frame(emm_size_p2$contrasts); df_emm_size_p2$Significance <- ifelse(df_emm_size_p2$p.value < alfa, "Significant", "Not Significant"); print(df_emm_size_p2); write.csv(df_emm_size_p2, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_coluna_traj_size", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
    } else {
     cat("Interaction is not significant. Proceeding to analyze main effects with a Type II ANOVA.\n")
      ...
      if (anova_size_type2["wave", "Pr(>Chisq)"] < 0.05) {cat("\n Main effect of 'wave' is significant. Performing contrasts.\n\n"); emm_size_wave <- emmeans(modelo_size, specs = pairwise ~ wave, adjust = "tukey"); df_emm_size_wave <- as.data.frame(emm_size_wave$contrasts); df_emm_size_wave$Significance <- ifelse(df_emm_size_wave$p.value < alfa, "Significant", "Not Significant"); print(df_emm_size_wave); write.csv(df_emm_size_wave, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_wave_size", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE) }
    ...
    cat("VIF:\n"); print(vif_df_size); write.csv(vif_df_size, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_vif_size", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
    #Random ou nao
    ...
    anova_conc <- Anova(modelo_conc, type = "III"); df_anova_conc <- as.data.frame(anova_conc); df_anova_conc$Significance <- ifelse(df_anova_conc$Pr(>Chisq) < alfa, "Significant", "Not Significant"); print(df_anova_conc); write.csv(df_anova_conc, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_anova_conc", tag_perc, "_", tag_transtorno, ".csv"))
    ...
    if (p_interacao_conc < 0.05) {
      cat("Interaction is significant. Performing contrasts for BOTH interaction perspectives.\n")
      emm_conc_p1 <- emmeans(modelo_conc, specs = pairwise ~ coluna_traj | wave, adjust = "tukey"); df_emm_conc_p1 <- as.data.frame(emm_conc_p1$contrasts); df_emm_conc_p1$Significance <- ifelse(df_emm_conc_p1$p.value < alfa, "Significant", "Not Significant"); print(df_emm_conc_p1); write.csv(df_emm_conc_p1, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_wave_conc", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
      emm_conc_p2 <- emmeans(modelo_conc, specs = pairwise ~ wave | coluna_traj, adjust = "tukey"); df_emm_conc_p2 <- as.data.frame(emm_conc_p2$contrasts); df_emm_conc_p2$Significance <- ifelse(df_emm_conc_p2$p.value < alfa, "Significant", "Not Significant"); print(df_emm_conc_p2); write.csv(df_emm_conc_p2, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_coluna_traj_conc", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
    } else {
      cat("Interaction is not significant. Proceeding to analyze main effects with a Type II ANOVA.\n")
      anova_conc_type2 <- Anova(modelo_conc, type = "II"); df_anova_conc <- as.data.frame(anova_conc_type2); df_anova_conc$Significance <- ifelse(df_anova_conc$Pr(>Chisq) < alfa, "Significant", "Not Significant"); print(df_anova_conc); write.csv(df_anova_conc, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_anova_concentration", tag_perc, "_", tag_transtorno, ".csv"))
      if (anova_conc_type2["wave", "Pr(>Chisq)"] < 0.05) {cat("\n Main effect of 'wave' is significant. Performing contrasts.\n\n"); emm_conc_wave <- emmeans(modelo_conc, specs = pairwise ~ wave, adjust = "tukey"); df_emm_conc_wave <- as.data.frame(emm_conc_wave$contrasts); df_emm_conc_wave$Significance <- ifelse(df_emm_conc_wave$p.value < alfa, "Significant", "Not Significant"); print(df_emm_conc_wave); write.csv(df_emm_conc_wave, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_wave_size", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE) }
    ...
    cat("VIF:\n"); print(vif_df_conc); write.csv(vif_df_conc, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_vif_concentration", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE);
    #Random ou nao
    ...
    print(tabela_diag_conc); write.csv(tabela_diag_conc, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_diagnostics_dharma_concentration", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
   ...
   if (tag_perc == "P90" && tag_transtorno == "dep")  {
     ...
     cat("\nVarrCorr:\n"); print(VarCorr(modelo_perc))
       ...
       png(paste0("analysis_results/", tag_perc, "", tag_transtorno, "/plot_diagnostics_perc_FIXO_NO_DISP", tag_perc, "_",  tag_transtorno, ".png"), width = 3000, height = 1200, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 0.8); plotQQunif(residuos_nodisp, main = "A) QQ Plot - PERC FIXO SEM DISP", testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_nodisp, main = ""); title("B) Residuals vs. Predicted – Perc (Fixo sem disp)", line = 2); dev.off()
    ...
    anova_perc_type3 <- Anova(modelo_perc, type = "III"); df_anova_perc3 <- as.data.frame(anova_perc_type3); df_anova_perc3$Significance <- ifelse(df_anova_perc3$Pr(>Chisq) < alfa, "Significant", "Not Significant"); print(df_anova_perc3); write.csv(df_anova_perc3, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_anova_percentage", tag_perc, "_", tag_transtorno, ".csv"))
    ...
    if (p_interacao_perc < 0.05) {
      ...
      emm_perc1 <- emmeans(modelo_perc, specs = pairwise ~ coluna_traj | wave, adjust = "tukey"); df_emm_perc1 <- as.data.frame(emm_perc1$contrasts); df_emm_perc1$Significance <- ifelse(df_emm_perc1$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc1); write.csv(df_emm_perc1, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_wave_perc", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
     emm_perc2 <- emmeans(modelo_perc, specs = pairwise ~ wave | coluna_traj, adjust = "tukey"); df_emm_perc2 <- as.data.frame(emm_perc2$contrasts); df_emm_perc2$Significance <- ifelse(df_emm_perc2$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc2); write.csv(df_emm_perc2, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_coluna_traj_perc", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
    } else {
    ...
    if (df_anova_perc2["wave", "Pr(>Chisq)"] < 0.05) {cat("\n Main effect of 'wave' is significant. Performing contrasts.\n\n"); emm_perc_wave <- emmeans(modelo_perc, specs = pairwise ~ wave, adjust = "tukey"); df_emm_perc_wave <- as.data.frame(emm_perc_wave$contrasts); df_emm_perc_wave$Significance <- ifelse(df_emm_perc_wave$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_wave); write.csv(df_emm_perc_wave, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_wave_percentage", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE) }
    if (df_anova_perc2["coluna_traj", "Pr(>Chisq)"] < 0.05) {cat("\n Main effect of 'coluna_traj' is significant. Performing contrasts.\n\n");emm_perc <- emmeans(modelo_perc, specs = pairwise ~ coluna_traj, adjust = "tukey"); df_emm_perc <- as.data.frame(emm_perc$contrasts); df_emm_perc$Significance <- ifelse(df_emm_perc$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc); write.csv(df_emm_perc, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_coluna_traj_percentage", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE) }
    if (anova_perc_type2["sex", "Pr(>Chisq)"] < 0.05) {cat("\n Main effect of 'sex' is significant. Performing contrasts.\n\n"); emm_perc_sex <- emmeans(modelo_perc, specs = pairwise ~ sex, adjust = "tukey"); df_emm_perc_sex <- as.data.frame(emm_perc_sex$contrasts); df_emm_perc_sex$Significance <- ifelse(df_emm_perc_sex$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_sex); write.csv(df_emm_perc_sex, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_sex_perc", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE) }
    if (anova_perc_type2["site", "Pr(>Chisq)"] < 0.05) {cat("\n Main effect of 'site' is significant. Performing contrasts.\n\n"); emm_perc_site <- emmeans(modelo_perc, specs = pairwise ~ site, adjust = "tukey"); df_emm_perc_site <- as.data.frame(emm_perc_site$contrasts); df_emm_perc_site$Significance <- ifelse(df_emm_perc_site$p.value < alfa, "Significant", "Not Significant"); print(df_emm_perc_site); write.csv(df_emm_perc_site, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_contrasts_site_perc", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE) }
    }
    ...
    cat("VIF:\n"); print(vif_df_perc); write.csv(vif_df_perc, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_vif_percentage", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
    #Random ou nao
    ...
    png(paste0("analysis_results/", tag_perc, "", tag_transtorno, "/plot_diagnostics_dharma_percentage", tag_perc, "_",  tag_transtorno, ".png"), width = 3000, height = 1200, res = 300); par(mfrow = c(1, 2), mar = c(4, 4, 3, 1) + 0.1, oma = c(0, 0, 1, 0), cex = 0.8); plotQQunif(residuos_perc, main = paste0("A) QQ Plot - percentage"), testUniformity = FALSE, testDispersion = FALSE, testOutliers = FALSE); plotResiduals(residuos_perc, main = ""); title("B) Residuals vs. Predicted - percentage", line = 2); dev.off()
    df_model <- model.frame(modelo_perc)
    vars <- c("sex", "site", "wave", "bage", coluna_traj)
    
    for(v in vars){
      # Define filename for each plot
      filename <-paste0("analysis_results/", tag_perc, "", tag_transtorno, "/plot_diagnostics_perc", v, "", tag_perc, "",  tag_transtorno, ".png")
    ...
    print(tabela_diag_perc); write.csv(tabela_diag_perc, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_diagnostics_dharma_percentage", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
    #AIC/BIC
    cat("\n\n=========================================================\n"); cat("  SUMMARY TABLE:AIC/BIC OF FINAL MODELS\n"); cat("=========================================================\n\n")
    print(resultados_finais_aic_bic)
    write.csv2(resultados_finais_aic_bic, paste0("analysis_results/", tag_perc, "", tag_transtorno, "/table_summary_aic_bic_final", tag_perc, "_", tag_transtorno, ".csv"), row.names = FALSE)
    #LRT
