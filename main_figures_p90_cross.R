...
nanosight_intersect_p90_cross <- read.csv("nanosight_intersect_p90_cross.csv")
...
# Graph A - Size
plot_a <- ggplot(nanosight_intersect_p90_cross, aes(x = grupo_analise_dep, y = tamanho_mode_average, fill = wave)) +
  ...
  labs(title = "A) EV's Average of Mode's Size", x = "Trajectory", y = "Size (nm)", fill = "Time point", color = "Time point") +
...
plot_b <- ggplot(nanosight_intersect_p90_cross, aes(x = grupo_analise_dep, y = concentracao_real, fill = wave)) +
  geom_boxplot(position = position_dodge(width = 0.8), outlier.shape = NA, alpha = 0.7, width = 0.6) +
  geom_jitter(aes(color = wave), position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.8), size = 2.2, alpha = 0.8) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40")
# Graph C - Small EV %
plot_c <- ggplot(nanosight_intersect_p90_cross, aes(x = grupo_analise_dep, y = EV_pequenas_porcentagem, fill = wave)) +
  geom_boxplot(position = position_dodge(width = 0.8), outlier.shape = NA, alpha = 0.7, width = 0.6) +
  geom_jitter(aes(color = wave), position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.8), size = 2.2, alpha = 0.8) +
